...
ok("the status reads the deposit account's template, not the card's", /rebuilt request/.test(byId.status.textContent), byId.status.textContent);
byId.account.value = "************1234";
byId.account.fire("change");
ok("choosing the card repaints on its own", /exact request copied/.test(byId.status.textContent), byId.status.textContent);
ok("switching to the card reports its own template", /exact request copied/.test(byId.status.textContent), byId.status.textContent);
...
ok("the button names the format", /Download CSV/.test(byId.run.textContent), byId.run.textContent);
byId.format.value = "json";
byId.format.fire("change");
ok("switching the format renames the button", /Download JSON/.test(byId.run.textContent), byId.run.textContent);
...
ok("says so", /copied as JSON/.test(byId.msg.textContent), byId.msg.textContent);
